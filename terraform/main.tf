...
resource "aws_ecs_cluster" "motif" {
  name = "motif"
}

resource "aws_ecr_repository" "motif" {
  name                 = "motif"
...
module "webserver" {
  ...
  ecr_repository = aws_ecr_repository.motif.repository_url
  ecs_cluster    = aws_ecs_cluster.motif.id
